...
- The disease id was converted from underscore to colon form (EFO_0020865 to EFO:0020865)
  and kept in its original ontology. This gene also links to MONDO-prefixed diseases in the
  sample (for example MONDO:0002026), showing the mixed disease ontologies that the mapping
  normalizes to CURIE form.

## MONDO disease name enrichment

Open Targets also uses MONDO disease ids. Before enrichment, the node name was the raw id:
...
What happened:
- `mondo_nodes.tsv` from the official release maps `MONDO:0002108` → `thyroid cancer`.
- 423 of 424 MONDO nodes in the graph received labels; one obsolete term (`MONDO:0700044`)
  was left as the raw id because obsolete entries are excluded from naming.
- EFO, DOID, Orphanet, and OTAR disease nodes are unchanged; only MONDO-prefixed nodes are
  enriched. Full report: `output/mondo_name_enrichment.csv`.
...
```
DRUG_NAME:TRAMETINIB --negatively_regulates--> NCBIGene:6416 --gene_associated_with_condition--> EFO:0020865
